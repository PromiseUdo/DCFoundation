...
const DomesticViolenceSupport = ({ domesticViolenceSupport }) => {
  return (
    <SectionContainer
      title={domesticViolenceSupport?.title}
      subtitle={domesticViolenceSupport?.subtitle}
      description=""
      hasBg={false}
    >
      <div className="w-full flex flex-col md:flex-row gap-12">
        <div className="w-full md:w-[50%] flex flex-col gap-2">
          <h3 className="text-3xl font-semibold mb-6 ">
            {domesticViolenceSupport?.aboutTitle}
          </h3>
          <p
            className="text-sm md:text-base leading-relaxed"
            dangerouslySetInnerHTML={{
              __html: domesticViolenceSupport?.aboutDescription,
            }}
          />

          <p className="text-sm md:text-base leading-relaxed mt-4">
            {domesticViolenceSupport?.programFeaturesTitle}
          </p>
          <ul className=" flex flex-col gap-2">
            {domesticViolenceSupport?.programFeaturesList?.map((item, i) => (
              <li
                key={i}
          ...
          <div className="w-full my-4">
            <button className="shadow-md bg-[#505ca4] text-[#f7f7f7] px-4 py-2 rounded-md text-base md:text-lg hover:bg-[#282e52] transition-all duration-100">
              {domesticViolenceSupport?.ctaCaption}
            </button>
          </div>
        </div>
        <div className="w-full md:w-[50%]  md:px-4">
          <h3 className="text-3xl font-semibold mb-6 ">
            {domesticViolenceSupport?.programBenefitsTitle}
          </h3>
          ...
          <div className="w-full h-[30rem]  ">
            <div className="overflow-y-auto h-full flex flex-col gap-2  scrollbar scrollbar-w-1   scrollbar-track-rounded-lg scrollbar-thumb-rounded-lg scrollbar-thumb-teal-80 scrollbar-track-gray-400">
              {domesticViolenceSupport?.benefitsList?.map((option, i) => (
                <Item
                  key={i}
                  title={option?.title}
